...
def init_db():
    ...
    # Migration: পুরনো DB তে নতুন column যোগ করো
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    elif data.startswith("model_ar_"):
        model_id = data[9:]
        upsert_user(user_id, model=model_id, provider=PROVIDER_AGENTROUTER)
        model_name = AGENTROUTER_MODELS.get(model_id, model_id)
        await query.edit_message_text(
    ...
    elif data.startswith("model_atxp_"):
        model_id = data[11:]
        db_user = get_user(user_id) or {}
        if not db_user.get("atxp_api_key"):
            await query.answer("⚠️ Set ATXP API key first! Use /setatxp YOUR_KEY", show_alert=True)
        else:
            upsert_user(user_id, model=model_id, provider=PROVIDER_ATXP)
            model_name = ATXP_MODELS.get(model_id, model_id)
            await query.edit_message_text(
                f"✅ *Model changed to:*\n`{model_name}`\n🔷 Provider: ATXP",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=models_kb(user_id),
    ...
    elif data.startswith("model_fw_"):
        short_key = data[9:]
        model_id = FIREWORKS_MODEL_MAP.get(short_key, short_key)
        db_user = get_user(user_id) or {}
        if not db_user.get("fireworks_api_key"):
            await query.answer("⚠️ Set Fireworks API key first! Use /setfw YOUR_KEY", show_alert=True)
        else:
            upsert_user(user_id, model=model_id, provider=PROVIDER_FIREWORKS)
            model_name = FIREWORKS_MODELS.get(model_id, model_id)
            await query.edit_message_text(
                f"✅ *Model changed to:*\n`{model_name}`\n🔥 Provider: Fireworks",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=models_kb(user_id),
...
async def handle_fireworks_api_key(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    key = update.message.text.strip()
    user_id = update.effective_user.id
    upsert_user(user_id, fireworks_api_key=key, provider=PROVIDER_FIREWORKS)
    await update.message.reply_text(
        "✅ *Fireworks API Key saved!*\n\nProvider set to 🔥 Fireworks.\nNow select a model from /models.",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=main_menu_kb(),
    )
    return ConversationHandler.END
...
async def handle_opencode_api_key(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    key = update.message.text.strip()
    user_id = update.effective_user.id
    upsert_user(user_id, opencode_api_key=key, provider=PROVIDER_OPENCODE)
    await update.message.reply_text(
        "✅ *OpenCode Zen API Key saved!*\n\nProvider set to 🆓 OpenCode Zen.\nNow select a model from /models.",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=main_menu_kb(),
    )
    return ConversationHandler.END
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    db_user = get_user(user_id) or {}
    provider = db_user.get("provider", DEFAULT_PROVIDER)

    if provider == PROVIDER_FIREWORKS:
        api_key = db_user.get("fireworks_api_key")
        if not api_key:
            await update.message.reply_text(
                "⚠️ Please set your Fireworks API key first!\nUse /setfw YOUR_KEY",
                reply_markup=main_menu_kb(),
            )
            return
    elif provider == PROVIDER_ATXP:
        api_key = db_user.get("atxp_api_key")
        if not api_key:
            await update.message.reply_text(
                "⚠️ Please set your ATXP API key first!\nUse /setatxp YOUR_KEY",
                reply_markup=main_menu_kb(),
            )
            return
    elif provider == PROVIDER_OPENCODE:
        api_key = db_user.get("opencode_api_key")
        if not api_key:
            await update.message.reply_text(
                "⚠️ Please set your OpenCode API key first!\nUse /setoc YOUR_KEY",
                reply_markup=main_menu_kb(),
            )
            return
    else:
        ...
        if not api_key:
            await update.message.reply_text(
                "⚠️ Please set your AgentRouter API key first!\nUse /setar YOUR_KEY",
                reply_markup=main_menu_kb(),
            )
            return
...
async def cmd_settings(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    upsert_user(user_id, update.effective_user.username)
    db_user = get_user(user_id) or {}
    ar_set = "✅ Set" if db_user.get("api_key") else "❌ Not Set"
    atxp_set = "✅ Set" if db_user.get("atxp_api_key") else "❌ Not Set"
    fw_set = "✅ Set" if db_user.get("fireworks_api_key") else "❌ Not Set"
    prompt_set = "✅ Custom" if db_user.get("system_prompt") else "📋 Default"
    provider = db_user.get("provider", DEFAULT_PROVIDER)
    if provider == PROVIDER_FIREWORKS:
        ...
        provider_label = "🌐 AgentRouter"
    await update.message.reply_text(
        f"⚙️ *Settings*\n\n"
        f"🌐 AgentRouter Key: {ar_set}\n"
        f"🔷 ATXP Key: {atxp_set}\n"
        f"🔥 Fireworks Key: {fw_set}\n"
        f"📡 Active Provider: {provider_label}\n"
        f"📝 System Prompt: {prompt_set}\n"
        f"🤖 Model: `{db_user.get('model', DEFAULT_MODEL)}`",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=settings_kb(),
...
async def cmd_help(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/setar `<key>` — Set AgentRouter API key\n"
        "/setatxp `<key>` — Set ATXP connection string\n"
        "/setfw `<key>` — Set Fireworks API key\n"
        "/help — Show this help\n"
        ...
        "🌐 *Providers:*\n"
        "• AgentRouter — Claude, DeepSeek, GLM\n"
        "• 🔷 ATXP — GPT-5.x, Claude 4.x\n"
        "• 🔥 Fireworks — Top coding models\n\n"
...
async def cmd_setar(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    upsert_user(user_id, update.effective_user.username)
    ...
    if len(parts) < 2:
        await update.message.reply_text(
            "❌ Usage: `/setar YOUR_API_KEY`\n\nExample: `/setar sk-abc123`",
            parse_mode=ParseMode.MARKDOWN,
        )
        return
    key = parts[1].strip()
    upsert_user(user_id, api_key=key)
    await update.message.reply_text(
        "✅ *AgentRouter API Key saved!*\n\nNow use /models to select a model.",
        parse_mode=ParseMode.MARKDOWN,
    )


async def cmd_setatxp(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    upsert_user(user_id, update.effective_user.username)
    full_text = update.message.text.strip()
    parts = full_text.split(None, 1)
    if len(parts) < 2:
        await update.message.reply_text(
            "❌ Usage: `/setatxp YOUR_CONNECTION_STRING`\n\nGet it from: https://accounts.atxp.ai",
            parse_mode=ParseMode.MARKDOWN,
        )
        return
    key = parts[1].strip()
    upsert_user(user_id, atxp_api_key=key)
    await update.message.reply_text(
        "✅ *ATXP API Key saved!*\n\nNow use /models to select an ATXP model.",
        parse_mode=ParseMode.MARKDOWN,
    )


async def cmd_setoc(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    upsert_user(user_id, update.effective_user.username)
    full_text = update.message.text.strip()
    parts = full_text.split(None, 1)
    if len(parts) < 2:
        await update.message.reply_text(
            "❌ Usage: `/setoc YOUR_API_KEY`\n\nGet it from: https://opencode.ai/auth",
            parse_mode=ParseMode.MARKDOWN,
        )
        return
    key = parts[1].strip()
    upsert_user(user_id, opencode_api_key=key, provider=PROVIDER_OPENCODE)
    await update.message.reply_text(
        "✅ *OpenCode Zen API Key saved!*\n\nProvider automatically set to 🆓 OpenCode Zen.\nNow use /models to select a free model.",
        parse_mode=ParseMode.MARKDOWN,
    )


async def cmd_setfw(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    upsert_user(user_id, update.effective_user.username)
    full_text = update.message.text.strip()
    parts = full_text.split(None, 1)
    if len(parts) < 2:
        await update.message.reply_text(
            "❌ Usage: `/setfw YOUR_API_KEY`\n\nGet it from: https://fireworks.ai/account/api-keys",
            parse_mode=ParseMode.MARKDOWN,
        )
        return
    key = parts[1].strip()
    ...
    await update.message.reply_text(
        "✅ *Fireworks API Key saved!*\n\nNow use /models to select a Fireworks model.",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def main():
    ...
    # পুরনো instance বন্ধ হওয়ার জন্য অপেক্ষা করো
    logger.info("Waiting for old instance to die...")
    await asyncio.sleep(5)

    # পুরনো connection clear করো (conflict prevent)
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(button_handler, pattern="^(set_api_key|set_atxp_api_key|set_fireworks_api_key|set_opencode_api_key|set_system_prompt|add_custom_model|menu_restore)$"),
            CommandHandler("restore", cmd_restore),
            CommandHandler("setar", cmd_setar),
            CommandHandler("setatxp", cmd_setatxp),
            CommandHandler("setoc", cmd_setoc),
            CommandHandler("setfw", cmd_setfw),
        ],
        per_message=False,
        states={
            ...
            WAITING_OPENCODE_API_KEY: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_opencode_api_key)],
    ...
    app.add_handler(CommandHandler("setar", cmd_setar))
    app.add_handler(CommandHandler("setatxp", cmd_setatxp))
    app.add_handler(CommandHandler("setfw", cmd_setfw))
    app.add_handler(CommandHandler("setoc", cmd_setoc))
    ...
    # Telegram command list সেট করো (autocomplete এর জন্য)
    await app.bot.set_my_commands([
        ("start", "Main menu"),
        ("menu", "Open menu"),
        ("models", "Select AI model"),
        ("settings", "Configure API keys"),
        ("setar", "Set AgentRouter API key"),
        ("setatxp", "Set ATXP connection string"),
        ("setfw", "Set Fireworks API key"),
        ("setoc", "Set OpenCode Zen API key"),
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if "Conflict" in str(e) and attempt < MAX_RETRIES - 1:
                wait = (attempt + 1) * 10
                logger.warning(f"Conflict detected, retrying in {wait}s... (attempt {attempt+1}/{MAX_RETRIES})")
                time.sleep(wait)
            else:
                raise
